...
editorproc(HWND window, UINT message, WPARAM wp, LPARAM lp)
{
	...
	if(message==WM_COMMAND && LOWORD(wp)==IDOK) {
		...
		if(!*p) { MessageBoxW(window,L"Enter a workspace name.",L"lti-ai",MB_OK); return 0; }
    ...
    wchar_t *name, size_t namecap, wchar_t *prompt, size_t promptcap)
{
	...
	wc.hCursor=LoadCursorW(NULL,IDC_ARROW); wc.hbrBackground=brush; wc.lpszClassName=L"LTIWorkspaceEditor";
